...
class DetailBymyself extends React.Component{
    constructor (...args) {
        super(...args)
        var id = '';
        for(var i=0;i<10;i++){
    ...
    componentDidMount () {
        setTimeout(()=>{
            if(location.href.indexOf('detailBymyself') >= 0){
                this.props.history.push('/alreadyBymyself');
            }
        },3000)
    }
    render () {
        ...
        return (
            <div className='detail'>
                ...
                <nav>
                    <p>待消费</p>
                    <span><button type='button'><Link to='/unBymyself'>退订</Link></button></span>
                </nav>
                <ul className='main'>
                    <li className='tit'>
                        <p>望湘园店</p>
...
export default {
    key: 'detailBymyself',
    path: '/detailBymyself',
    component: connect(slect)(DetailBymyself)
}